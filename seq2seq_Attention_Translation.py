...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        attn_applied = torch.bmm(attn_weights.unsqueeze(0),
                                 encoder_outputs.unsqueeze(0)) # 내가 볼땐 이 부분이 내가 배운 weight 부분. location base
...
savePath_en = "./output_model/en_model.pth"
savePath_de = "./output_model/de_model.pth"
#torch.save(encoder1.state_dict(), savePath_en)
#torch.save(attn_decoder1.state_dict(), savePath_de)


encoder1 = EncoderRNN(input_lang.n_words, hidden_size)
attn_decoder1 = AttnDecoderRNN(hidden_size, output_lang.n_words, dropout_p=0.1)
encoder1.load_state_dict(torch.load(savePath_en))
attn_decoder1.load_state_dict(torch.load(savePath_de))
